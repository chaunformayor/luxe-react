...
export async function getOwnerMaintenanceRequests(ownerId: string) {
  ...
  try {
    ...
    return await db
      ...
      .where(inArray(maintenanceRequests.propertyId, propIds));
  } catch (error) {
...
export async function getOwnerStats(ownerId: string) {
  ...
  try {
    ...
    const [tenantsCount, maintenanceCount, revenueResult] = await Promise.all([
      unitIds.length
        ? db.select({ count: count() }).from(tenants).where(inArray(tenants.unitId, unitIds))
      ...
      unitIds.length
        ? db.select({ total: sum(payments.amount) }).from(payments).where(and(inArray(payments.unitId, unitIds), eq(payments.status, "completed")))
